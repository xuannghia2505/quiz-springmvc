...
<div style="display: flex; margin-top: 50px;">
	...
	<form action="/quiz/search" method="post"
		class="form-inline form-search-custom"
		style="margin-left: 440px; margin-bottom: 20px;">
		<input class="form-control mr-sm-2" type="search"
			placeholder="Tìm kiếm Quiz" aria-label="Search" name="keySearch">
		<button class="btn btn-primary my-2 my-sm-0" type="submit">Search</button>
		<button class="btn btn-success ml-2 " type="button"
			onclick="createForm()">Create</button>
	</form>
	<span id="titleEdit"
		style="font-size: 32px; font-weight: 600; margin-left: 30px;"
		class="hide"> Edit Quiz</span> <span id="titleCreate" class="hide"
		style="font-size: 32px; font-weight: 600; margin-left: 30px;">
		Create Quiz</span>
</div>
<div class="container-fluid" style="display: flex;">
			...
			<tbody>
				<c:forEach items="${listQuizs}" var="quiz">
					...
					<tr style="text-align: center;">
						...
						<td>${quiz.catelogy }</td>
						<td><img
							src="${pageContext.request.contextPath}/${quiz.image}" width="50"
							height="50" /></td>
						<td>${quiz.numberQuestion }</td>
						<td><a class="btn btn-primary"
							href="/question/editQuestion?quizID=${quiz.quizID}"><i
								class="fa fa-arrow-right"></i></a>
							<button class="btn btn-success"
								onclick="getQuiz('${quiz.quizID}')">
	...
	<form action="/quiz/create" method="post" class="hide"
		id="formCreateQuiz" style="align-items: center; margin-left: 30px;"
		onsubmit="return Validation();" enctype="multipart/form-data">
		<div class="form-group">
			<b>Name</b> <input type="text" class="form-control"
				style="width: 350px;" name="quizName" placeholder="Name"
				id="quizName">

		</div>
		<small style="color: red" id="errorName"></small>
		<div class="form-group">
			<b>Catelogy</b> <select class="form-control" name="catelogy"
				id="selectCatelogy">
