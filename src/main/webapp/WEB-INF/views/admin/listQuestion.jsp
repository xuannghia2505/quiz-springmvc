		...
		<select class="form-control" name="quizID" id="selectCatelogy"
			...
			<c:if test="${quiz.catelogy!='quizenglish'}">
				<button style="margin-left: 10px;" type="button"
					...
					data-target="#modalCreateQuestion1">Add New Question Text</button>

				<button style="margin-left: 10px;" type="button"
					class="btn btn-primary" id="btnAddQuestion" data-toggle="modal"
					data-target="#modalCreateQuestion3">Add New Question Image
					And Audio</button>
			</c:if>
			<c:if test="${quiz.catelogy=='quizenglish'}">
				<button style="margin-left: 10px;" type="button"
					class="btn btn-warning" id="btnAddQuestion" data-toggle="modal"
	...
	<table class="table table-hover">
			...
			<tbody>
				<c:forEach items="${listQuestions}" var="question">
					...
					<tr style="text-align: center;" id="contentEdit">
						<td>${question.questionID }</td>
						<td><c:if test="${question.question!=null }">
					${question.question }
					
					</c:if></td>

						<td><c:if test="${question.answerA!=null }">
					...
					${question.answerA }
						...
						<td><c:if test="${question.answerB!=null }">
								<c:if test="${question.questionType=='text' }">
					${question.answerB }
					</c:if>
								<c:if test="${question.questionType=='imageAndaudio' }">
						...
						<td><c:if test="${question.answerC!=null }">
								<c:if test="${question.questionType=='text' }">
					${question.answerC }
					</c:if>
								<c:if test="${question.questionType=='imageAndaudio' }">
						...
						<td><c:if test="${question.answerD!=null }">
								<c:if test="${question.questionType=='text' }">
					${question.answerD }
					</c:if>
								<c:if test="${question.questionType=='imageAndaudio' }">
						...
						<td><c:if test="${question.correctAnswer!=null }">
					${question.correctAnswer }
					</c:if></td>
						<td><c:if test="${question.image!=null }">
					<img src="${question.image }" width="50" height="50">
					</c:if></td>
						<td><c:if test="${question.audio!=null }">
						<c:if test="${question.questionType=='text' }">
					${question.audio }
					</c:if>
								<c:if test="${question.questionType=='imageAndaudio' }">
									<audio src="/${question.audio }" controls style="width:100px;"></audio>
								</c:if>
									...
									<div class="modal-body">
										...
										<div class="form-group">
											<b>Quiz</b> <br /> <select class="form-control"
												...
												<c:forEach items="${listQuizsI}" var="quiz">
													<option value="${quiz.quizID}"
														${quiz.quizID==quizID ? 'selected="selected"' : ''}>${quiz.name}</option>
												</c:forEach>
											</select>
						...
						<div class="modal-dialog" role="document">
							<div class="modal-content">
								...
								<form action="/question/update2" method="post"
									enctype="multipart/form-data">
									<input value="${question.questionID}" name="questionID" hidden>
									<input value="${question.quizID}" name="quizID" hidden>
									<div class="modal-body">
										<div style="text-align: center; font-size: 22px;">
											Bạn đang chỉnh sửa Question ID: <b style="color: red;">${question.questionID}</b>
										</div>
										...
										<div class="form-group">
											...
											<div style="display: flex;">
												<img src="${question.image}" width="50" height="50"
													...
													id="newEditImageEnglish" class="hide"> <input
													type="file" class="form-control" name="image"
													onchange="readURLEditImageEnglish(this);">
											</div>
										</div>
					...
					<c:if test="${question.questionType=='imageAndaudio'}"><div class="modal fade"
						id="modalEditQuestion${question.questionID }" tabindex="-1"
						role="dialog"
						aria-bledby="exampleModalbEdit${question.questionID }"
						aria-hidden="true">
...
<div class="modal fade" id="modalCreateQuestion1" tabindex="-1"
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<form action="/question/add1" method="post"
				onsubmit="return ValidationAdd1();" )>
				<input value="${quizID}" name="quizID" hidden>
				<input value="text" name="questionType" hidden>
				<div class="modal-body">
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<form action="/question/add2" method="post"
				...
				<input value="${quizID}" name="quizID" hidden>
				<input value="textAndimage" name="questionType" hidden>
				<div class="modal-body">
					<div class="form-group">
						<b for="exampleInput1">Question</b> <input type="text"
							class="form-control" id="exampleInput1"
					...
					<div class="form-group">
						<b>Image</b> <br />
						<div style="display: flex;">
							<img width="50" height="50" id="newAddImageEnglish" class="hide">
							<input id="imageQuestion2" type="file" class="form-control"
								name="image" onchange="readURLAddImageEnglish(this);">
						</div>
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<form action="/question/add3" method="post"
				...
				<input value="${quizID}" name="quizID" hidden>
				<input value="imageAndaudio" name="questionType" hidden>
				<div class="modal-body">
					...
					<div class="form-group">
						...
						<div style="display: flex;">
							<img id="newImageAdd1" width="50" height="50" class="hide">
							<input type="file" class="form-control" id="exampleInputAdd1"
								placeholder="Nhập câu trả lời A" name="answerA"
					...
					<div class="form-group">
						...
						<div style="display: flex;">
							<img id="newImageAdd2" width="50" height="50" class="hide">
							<input type="file" class="form-control" id="exampleInputAdd2"
								placeholder="Nhập câu trả lời B" name="answerB"
					...
					<div class="form-group">
						...
						<div style="display: flex;">
							<img id="newImageAdd3" width="50" height="50" class="hide">
							<input type="file" class="form-control" id="exampleInputAdd3"
								placeholder="Nhập câu trả lời C" name="answerC"
					...
					<div class="form-group">
						...
						<div style="display: flex;">
							<img id="newImageAdd4" width="50" height="50" class="hide">
							<input type="file" class="form-control" id="exampleInputAdd4"
								placeholder="Nhập câu trả lời D" name="answerD"
					...
					<div class="form-group">
						<b for="exampleInput5">Correct Answer</b> <select
							...
							<option value="C">C</option>
							<option value="D">D</option>
						</select> <small style="color: red" id="exampleInputError5"></small>
					</div>
					<div class="form-group">
...
<script>
	...
	function readURLAddImageEnglish(input) {
		if (input.files && input.files[0]) {
			document.getElementById('newAddImageEnglish').classList
					.remove("hide");
			var reader = new FileReader();

			reader.onload = function(e) {
				$('#newAddImageEnglish').attr('src', e.target.result)
						.height(50);
			};
	...
	function readURLEditImageEnglish(input) {
		if (input.files && input.files[0]) {
			document.getElementById('oldEditImageEnglish').classList
					.add("hide");
			document.getElementById('newEditImageEnglish').classList
					.remove("hide");
			var reader = new FileReader();

			reader.onload = function(e) {
				$('#newEditImageEnglish').attr('src', e.target.result).height(
						50);
			};
